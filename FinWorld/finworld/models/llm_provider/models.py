...
from finworld.models.llm_provider.litellm import LiteLLMModel, AzureLiteLLMModel
from finworld.models.llm_provider.openaillm import OpenAIServerModel
from finworld.models.llm_provider.hfllm import InferenceClientModel
from finworld.models.llm_provider.restful import RestfulModel
from finworld.utils import Singleton
from finworld.proxy.local_proxy import HTTP_CLIENT, ASYNC_HTTP_CLIENT
...
class ModelManager(metaclass=Singleton):
    ...
    def init_models(self, use_local_proxy: bool = False):
        ...
        self._register_azure_openai_models(use_local_proxy=use_local_proxy)
    ...
    def _check_local_api_version(self, local_api_version_name: str, remote_api_version_name: str) -> str:
        api_version = os.getenv(local_api_version_name, PLACEHOLDER)
        if api_version == PLACEHOLDER:
            logger.warning(f"Local API version {local_api_version_name} is not set, using remote API version {remote_api_version_name}")
            api_version = os.getenv(remote_api_version_name, PLACEHOLDER)
        return api_version
    ...
    def _register_azure_openai_models(self, use_local_proxy: bool = False):
        if use_local_proxy:
            logger.warning("Azure OpenAI models are not supported with local proxy yet.")
        else:
            logger.info("Using remote API for Azure OpenAI models")
            api_key = self._check_local_api_key(local_api_key_name="AZURE_OPENAI_API_KEY",
                                                remote_api_key_name="AZURE_OPENAI_API_KEY")
            api_base = self._check_local_api_base(local_api_base_name="AZURE_OPENAI_API_BASE",
                                                    remote_api_base_name="AZURE_OPENAI_API_BASE")
            api_version = self._check_local_api_version(local_api_version_name="AZURE_OPENAI_API_VERSION",
                                                        remote_api_version_name="AZURE_OPENAI_API_VERSION")
