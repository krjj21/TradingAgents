...
class DLinear(nn.Module):
    def __init__(self,
        ...
        super(DLinear, self).__init__(*args, **kwargs)
        ...
        self.linear_seasonal = nn.Linear(
            in_features=history_timestamps,
            out_features=future_timestamps
        )
        self.linear_seasonal.weight = nn.Parameter(
            (1 / history_timestamps) * torch.ones([future_timestamps, history_timestamps])
        )
        ...
        self.proj = nn.Linear(
            in_features=self.encoder_embed.output_dim,
    ...
    def encode(self, features: torch.Tensor, times: torch.Tensor) -> torch.Tensor:
        ...
        seasonal_init = seasonal_init.permute(0, 2, 1)  # (batch_size, num_features, seq_length)
        trend_init = trend_init.permute(0, 2, 1)  # (batch_size, num_features, seq_length)

        seasonal_output = self.linear_seasonal(seasonal_init)  # (batch_size, num_features, future_timestamps)
        trend_output = self.linear_trend(trend_init)  # (batch_size, num_features, future_timestamps)

        x = seasonal_output + trend_output  # (batch_size, num_features, future_timestamps)
        x = x.permute(0, 2, 1)  # (batch_size, future_timestamps, num_features)
    ...
    def forward_classification(self, x: TensorDict, target: TensorDict) -> torch.Tensor:
        ...
        encode_output = encode_output.mean(dim=1)  # Global average pooling over time dimension
