...
class PPOTradingTrainer():
    ...
    def __str__(self):
        return f"PPOTradingTrainer(task_type={self.config.task_type})"
    ...
    def train(self):
        ...
        for update in range(1, num_updates + 1 + warm_up_updates):
            ...
            metrics = {
                ...
                f"{mode}/policy_loss": trading_records["pg_loss"].item(),
                f"{mode}/value": trading_records["value"].item(),
                f"{mode}/logprob": trading_records["logprob"].item(),
                f"{mode}/entropy": trading_records["entropy_loss"].item(),
                f"{mode}/old_approx_kl": trading_records["old_approx_kl"].item(),
                f"{mode}/approx_kl": trading_records["approx_kl"].item(),
                f"{mode}/total_approx_kl": trading_records["total_approx_kl"].item(),
                f"{mode}/policy_update_times": trading_records["policy_update_steps"]// self.config.gradient_checkpointing_steps,
