...
class SACPortfolioTrainer():
    ...
    def __str__(self):
        return f"SACPortfolioTrainer(task_type={self.config.task_type})"
    ...
    def explore_environment(self,
        ...
        for step in range(0, self.config.num_steps):
            ...
            if self.global_step < self.config.learning_start:
                dirichlet = torch.distributions.Dirichlet(torch.ones(self.config.action_dim))
                action = dirichlet.sample((self.config.num_envs,)).to(self.device)
            else:
                    ...
                    input_tensor = TensorDict(
                        {
                            ...
                            "positions": next_obs["policy_portfolio_positions"],
                            "actions": next_obs["policy_portfolio_actions"],
            ...
            real_next_obs = TensorDict({
                ...
                "next_policy_portfolio_cashes": next_obs["policy_portfolio_cashes"],
                "next_policy_portfolio_positions": next_obs["policy_portfolio_positions"],
                "next_policy_portfolio_actions": next_obs["policy_portfolio_actions"],
                "next_policy_portfolio_rets": next_obs["policy_portfolio_rets"],
            }, batch_size=self.config.num_envs).to(self.device)
        ...
        for start in range(0, self.config.batch_size, self.config.value_minibatch_size):
            ...
            with torch.no_grad():
                input_tensor = TensorDict({
                    ...
                    "cashes": flat_storage["next_policy_portfolio_cashes"][mb_inds],
                    "positions": flat_storage["next_policy_portfolio_positions"][mb_inds],
                    "actions": flat_storage["next_policy_portfolio_actions"][mb_inds],
                    "rets": flat_storage["next_policy_portfolio_rets"][mb_inds],
            ...
            input_tensor = TensorDict({
                ...
                "positions": flat_storage["policy_portfolio_positions"][mb_inds].to(torch.int32),
                "actions": flat_storage["policy_portfolio_actions"][mb_inds].to(torch.int32),
                "rets": flat_storage["policy_portfolio_rets"][mb_inds],
            }, batch_size=self.config.value_minibatch_size).to(self.device)

            q1, q2 = self.agent.get_value(input_tensor, use_target=False)

            q1_value = q1.sum(dim=-1)
            q2_value = q2.sum(dim=-1)
    ...
    def update_policy(self,
        ...
        for start in range(0, self.config.batch_size, self.config.policy_minibatch_size):
            ...
            _, action_probs, log_prob= self.agent.get_action_and_value(input_tensor)
                ...
                input_tensor = TensorDict(
                    {
                        "dense": flat_storage["features"][mb_inds],
                        "sparse": flat_storage["times"][mb_inds].to(torch.int32),
                        "cashes": flat_storage["expert_portfolio_cashes"][mb_inds],
                        "positions": flat_storage["expert_portfolio_positions"][mb_inds].to(torch.int32),
                        "actions": flat_storage["expert_portfolio_actions"][mb_inds].to(torch.int32),
                        "rets": flat_storage["expert_portfolio_rets"][mb_inds],
                    }, batch_size=self.config.policy_minibatch_size).to(self.device
                                                                        )
                input_actions = flat_storage["training_expert_actions"][mb_inds].unsqueeze(-1).long()
                expert_logits = self.agent.actor(input_tensor)
                bc_loss = nn.functional.mse_loss(expert_logits, input_actions.flatten())
        ...
        for update in range(1, num_updates + 1 + warm_up_updates):
            ...
            if self.global_step > self.config.learning_start:
                ...
                metrics = {
                    ...
                    f"{mode}/policy_loss": portfolio_records["a_loss"].item(),
                    f"{mode}/alpha_loss": portfolio_records["alpha_loss"].item(),
