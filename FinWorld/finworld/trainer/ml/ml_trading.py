...
class MLTradingTrainer(Trainer):
    def __init__(self,
                 config = None,
                 dataset=None,
                 metrics = None,
        ...
        super(MLTradingTrainer, self).__init__(**kwargs)
        ...
        self.data_columns = dataset.asset_meta_info['columns']
    ...
    def train(self):
        ...
        logger.info(f"| Training MLTradingAgent...")

        train_data = self.train_environment.get_data()
        valid_data = self.valid_environment.get_data()

        features_df = train_data["features_df"]
        labels_df = train_data["labels_df"]
        ...
        data["labels"] = labels_df[ret_labels]

        valid_features_df = valid_data["features_df"]
        valid_labels_df = valid_data["labels_df"]
        valid_data = {}
        valid_data["features"] = valid_features_df
        valid_data["labels"] = valid_labels_df[ret_labels]
    ...
    def valid(self):
        ...
        logger.info(f"| Validating MLTradingAgent...")
    ...
    def test(self):
        ...
        logger.info(f"| Testing MLTradingAgent...")
