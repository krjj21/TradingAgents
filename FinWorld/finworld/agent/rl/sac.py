...
class SAC(nn.Module):
    ...
    def get_action_and_value(self, x: TensorDict, a: Tensor = None):
        ...
        if self.task_type == TaskType.TRADING:
            ...
            action_probs = dis.probs
            log_prob = F.log_softmax(logits, dim=-1)
            
            if len(batch_size) == 2:
                action = rearrange(a, "(b c) -> b c", b=batch_size[0], c=batch_size[1])
                action_probs = rearrange(action_probs, "(b c) n -> b c n", b=batch_size[0], c=batch_size[1])
                log_prob = rearrange(log_prob, "(b c) n -> b c n", b=batch_size[0], c=batch_size[1])
            else:
                action = a
...
if __name__ == '__main__':
    ...
    output1, output2 = sac.get_value(x)
    print("Output1 shape:", output1.shape)
    print("Output2 shape:", output2.shape)
    action, action_probs, log_prob = sac.get_action_and_value(x)
    ...
    dense_features = torch.randn(batch_size, seq_len, num_assets,
                                 64)  # Batch size of 2, 10 time steps, num_asset assets, 64 features
    years = torch.randint(0, 10, (batch_size, seq_len, 1))  # Year feature
    months = torch.randint(1, 13, (batch_size, seq_len, 1))  # Month feature
    weekdays = torch.randint(0, 7, (batch_size, seq_len, 1))  # Weekday feature
    days = torch.randint(1, 32, (batch_size, seq_len, 1))  # Day feature
